...
#
# On the audit: it greps for the bare version string, so its noise level depends
# on how common that string happens to be. `4.2.0` matched almost nothing; `5.0.0`
# matched 21 unrelated npm dependency ranges under examples/. That is why
# `examples` is in audit.exclude — those are standalone demo projects generated by
# the pipeline, with their own independent versioning. Bumping them alongside the
# plugin would be wrong, so the audit has no reason to look there. Do not remove
# that exclude to "widen coverage"; it hides nothing.
#
# CHANGELOG.md is expected to show up as undeclared on every release: this script
# bumps the manifests but not the changelog heading, which is written by hand.
# That one line is the reminder to write it.
#
set -euo pipefail
