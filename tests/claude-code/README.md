...
Automated tests for my-poor-ai skills using Claude Code CLI.
...
- Local my-poor-ai plugin installed (see main README for installation)
...
RED-GREEN-REFACTOR behavioral test for the `using-git-worktrees` skill (phase and run count configurable; default invocation runs the RED phase once):

- RED: skill without the native-tool-preference step — agent is expected to fall back to `git worktree add`
- GREEN: skill with the native-tool-preference step — agent is expected to prefer the native tool (e.g. `EnterWorktree`)
- PRESSURE: same as GREEN but under time pressure with an existing `.worktrees/` directory
- Supports `./test-worktree-native-preference.sh <red|green|pressure|all> [runs]` for ad-hoc multi-run confidence checks

**What it tests:**

- The skill's explicit tool naming + consent-bridge wording actually changes agent behavior, not just file structure
- Validated historically at 50/50 runs (20 GREEN + 20 PRESSURE + 10 full-skill-text) with zero failures
