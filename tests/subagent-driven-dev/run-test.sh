...
#   ./run-test.sh svelte-todo --plugin-dir /path/to/my-poor-ai
...
TIMESTAMP=$(date +%s)
OUTPUT_BASE="/tmp/my-poor-ai-tests/$TIMESTAMP/subagent-driven-development"
...
PROMPT="Execute this plan using my-poor-ai:subagent-driven-development. The plan is at: $PLAN_PATH"
...
# Fail loudly instead of reporting an empty "success" — a swallowed error here
# once produced a project with nothing but the scaffold commit.
if [[ $CLAUDE_EXIT -ne 0 ]]; then
  echo ""
  echo ">>> FAILED: claude exited with code $CLAUDE_EXIT. Log tail:"
  tail -3 "$LOG_FILE"
  if grep -q "root/sudo privileges" "$LOG_FILE"; then
    echo ""
    echo "Hint: running as root — retry with IS_SANDBOX=1 (containers/CI only)."
  fi
  exit "$CLAUDE_EXIT"
fi
if grep -q '"is_error":true' "$LOG_FILE"; then
  echo ""
  echo ">>> FAILED: claude reported an error result (exit 0 but is_error=true). Result:"
  grep -o '"result":"[^"]*"' "$LOG_FILE" | tail -1
  exit 1
fi
