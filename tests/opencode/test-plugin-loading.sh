#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the my-poor-ai plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/my-poor-ai.js"
...
# Test 3: Check using-my-poor-ai skill exists (critical for bootstrap)
echo "Test 3: Checking using-my-poor-ai skill (required for bootstrap)..."
if [ -f "$FORGE_SKILLS_DIR/using-my-poor-ai/SKILL.md" ]; then
    echo "  [PASS] using-my-poor-ai skill exists"
else
    echo "  [FAIL] using-my-poor-ai skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/my-poor-ai/' "$FORGE_PLUGIN_FILE"; then
