...
digraph when_to_use {
    ...
    "서브에이전트를 쓸 수 있는가?" [shape=diamond];
    "subagent-driven-development" [shape=box];
    "인라인 실행 (아래 폴백)" [shape=box];
    "수동 실행 또는 먼저 brainstorming" [shape=box];

    "구현 플랜이 있는가?" -> "작업이 대체로 독립적인가?" [label="예"];
    "구현 플랜이 있는가?" -> "수동 실행 또는 먼저 brainstorming" [label="아니오"];
    "작업이 대체로 독립적인가?" -> "서브에이전트를 쓸 수 있는가?" [label="예"];
    "작업이 대체로 독립적인가?" -> "수동 실행 또는 먼저 brainstorming" [label="아니오 - 강결합"];
    "서브에이전트를 쓸 수 있는가?" -> "subagent-driven-development" [label="예"];
    "서브에이전트를 쓸 수 있는가?" -> "인라인 실행 (아래 폴백)" [label="아니오"];
}
```
...
**작업을 하나라도 시작하기 전에 플랜 전체를 읽고 비판적으로 검토함.** 읽어서 작업을 추출하는 것과, 플랜이 실행 가능한지 판단하는 것은 다른 일임.

1. 플랜 파일을 끝까지 읽음
2. 질문·우려 사항을 식별 — 착수를 막는 공백, 모순된 단계, 검증 불가능한 완료 조건
3. **우려가 있으면 착수 전에 사용자에게 제기함.** 실행하면서 발견하겠다고 미루지 않음
4. 우려가 없으면 전체 작업으로 TodoWrite를 생성하고 진행

착수 후 다음 상황에서는 이 검토 단계로 **되돌아감**:

- 피드백에 따라 사용자가 플랜을 갱신한 경우
- 근본적인 접근 방식 재검토가 필요한 경우

## 서브에이전트를 쓸 수 없는 환경 (폴백)

Claude Code에는 항상 서브에이전트가 있으므로 이 절은 그 외 플랫폼용임. 서브에이전트가 없으면 위임·2단계 검토가 성립하지 않으므로 **인라인으로 실행**함:

1. 위 "착수 전 플랜 비판적 검토"를 동일하게 수행
2. 작업마다: `in_progress` 표시 → 플랜의 단계를 정확히 따름 → 명시된 검증 수행 → `completed` 표시
3. 서브에이전트 검토가 없는 만큼 `my-poor-ai:verification-before-completion`의 게이트를 작업 단위로 직접 적용함
4. 완료 후 `my-poor-ai:finishing-a-development-branch`로 마무리

**사용자에게 고지할 것:** 서브에이전트를 지원하는 플랫폼에서 실행하면 작업 품질이 현저히 높아짐.

인라인 실행에서도 HANDOFF.md 규약과 아래 "도움 요청 시점"은 동일하게 적용됨.

## 도움 요청 시점

**다음 상황에서 즉시 실행을 중단하고 질문함:**

- 블로커 발생 (누락된 의존성, 테스트 실패, 지시 사항 불명확)
- 플랜에 착수를 막는 심각한 공백 존재
- 검증이 반복적으로 실패

**추측으로 통과하지 않음.** 블로커를 강제로 밀고 나가는 것은 연속 실행 원칙의 예외임 — 연속 실행은 "확인차 멈추지 말 것"이지 "막혀도 계속하라"가 아님.
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "구현자: TDD(테스트 선행) → 커밋 → 보고" [shape=box];
    ...
    "구현자가 질문하는가?" -> "구현자: TDD(테스트 선행) → 커밋 → 보고" [label="아니오"];
    "구현자: TDD(테스트 선행) → 커밋 → 보고" -> "스펙 준수 검토자 배포 (./spec-reviewer-prompt.md)";
...
**추론 강도(effort) 축:** 모델 등급과 별개로 조절 가능한 축임. 등급을 낮추는 것보다 강도를 낮추는 편이 품질 손실이 적은 경우가 많으므로, 등급을 내리기 전에 강도부터 조절함.

- 기계적 구현·정형 수정 → 낮은 강도
- 통합·디버깅 → 중간 강도
- 설계 판단·검토 → 높은 강도

강도를 지원하는 환경에서만 적용함. 지원하지 않으면 위의 모델 등급 축만 씀. 강도를 높게 두는 경우에는 출력 토큰 상한을 넉넉히 잡음 — 추론이 예산을 잠식해 답변이 잘리는 것을 막기 위함임.
...
[나중에] 구현자:
  ...
  - 테스트 추가, 5/5 통과
  - 구현 중 --force 플래그 누락을 발견해 그 자리에서 추가
  - 커밋 완료
...
- 2단계 검토: 스펙 준수 후 코드 품질 — 검토 주체가 구현자와 분리되어 있음(writer-verifier)
- 검토 루프로 수정 실제 동작 보장
- 스펙 준수로 과잉/과소 구현 방지
...
- 구현자에게 별도 자체 검토 라운드를 강제하기 — 전담 검토자 2명이 같은 항목을 보므로 중복임 (`verification-before-completion`의 「중복 검증과의 구분」 참조)
...
**서브에이전트가 없는 플랫폼:** 위 "서브에이전트를 쓸 수 없는 환경 (폴백)" 절을 따름.
