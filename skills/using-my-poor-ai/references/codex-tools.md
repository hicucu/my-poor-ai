...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `Agent` 도구 (서브에이전트 파견) | `spawn_agent` ([서브에이전트 파견에는 멀티 에이전트 지원 필요](#서브에이전트-파견에는-멀티-에이전트-지원-필요) 참조) |
| 여러 `Agent` 호출 (병렬)        | 여러 `spawn_agent` 호출                                                                                              |
| Agent 결과 반환                 | `wait_agent`                                                                                                         |
| Agent 자동 완료                 | `close_agent`로 슬롯 해제                                                                                            |
