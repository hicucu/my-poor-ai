...
판정은 2단 — 목표가 잡혀 있는가, 방향이 정해져 있는가:

```dot
digraph direction_gate {
    "목표(무엇을 하려는지)가 한 줄로 잡히는가?" [shape=diamond];
    "산파술 질문으로 목표 추리기" [shape=box];
    "방향(어떤 방식으로)이 정해져 있는가?" [shape=diamond];
    "which-way-should-i-go (3렌즈 조사)" [shape=box];
    "my-poor-ai:brainstorming" [shape=box];
    "사용자에게 확인: '방향은 정해진 상태인가요?'" [shape=box];

    "목표(무엇을 하려는지)가 한 줄로 잡히는가?" -> "산파술 질문으로 목표 추리기" [label="아니오"];
    "산파술 질문으로 목표 추리기" -> "목표(무엇을 하려는지)가 한 줄로 잡히는가?" [label="잡힐 때까지"];
    "목표(무엇을 하려는지)가 한 줄로 잡히는가?" -> "방향(어떤 방식으로)이 정해져 있는가?" [label="예"];
    "방향(어떤 방식으로)이 정해져 있는가?" -> "which-way-should-i-go (3렌즈 조사)" [label="아니오"];
    "방향(어떤 방식으로)이 정해져 있는가?" -> "my-poor-ai:brainstorming" [label="예"];
    "방향(어떤 방식으로)이 정해져 있는가?" -> "사용자에게 확인: '방향은 정해진 상태인가요?'" [label="애매함"];
}
```

**애매하면 추측하지 않고 사용자에게 직접 물음.** 사용자가 특정 기술·방식을 언급했더라도 그것이 확정인지 검토 중인 후보인지 불분명하면 애매함으로 취급함.

### 목표 자체가 불명확할 때: 산파술로 추리기

"무엇을 하려는지"조차 흐릿하면 (예: "AI로 뭔가 해보고 싶은데 뭘 할지 모르겠어") 바로 3렌즈 조사에 들어가지 않음 — 목표 없는 조사는 일반론만 낳음. `my-poor-ai:socratic-plan-review`의 산파술 원칙(답을 대신 쓰지 않고 질문이 완성시킴)을 목표 추리기에 차용함:

- 한 번에 하나씩 질문해 사용자의 생각을 끌어냄: 해결하고 싶은 불편이나 노리는 기회는? 누구를 위한 것인가? 완성되면 무엇이 가능해지는가?
- 선택지를 대신 제안하기보다 사용자가 이미 가진 생각을 표면화함
- **목표가 한 줄로 잡히는 즉시 중단**하고 방향 판정으로 넘어감 — 요구사항 전체를 캐지 않음 (그건 brainstorming의 몫)
...
### 변형: 신생 시장 (렌즈 A가 성립하지 않음)

도메인 자체가 갓 등장한 시장이라 "오래 검증된 주류"가 아예 없으면 세대 프레임이 성립하지 않음. 이때는 세대 구분을 접고 **지금 가장 핫한 접근법 3가지**를 비교함:

1. 빠른 웹 검색 한 번으로 현재 경쟁 중인 접근법들을 훑어 가장 핫한 3가지를 선정
2. 접근법당 에이전트 1개씩 병렬 디스패치 — 프롬프트의 렌즈 자리에 "담당 접근법: {이름}"을 넣고 나머지(웹 검색 지시·반환 형식)는 동일

판단 기준: 렌즈 A 후보가 떠오르지 않고, 검색해도 수년 이상 검증된 방식이 나오지 않으면 신생 시장으로 취급함. 인접 도메인의 레거시(예: 일반 웹 서비스 구축 방식)를 억지로 렌즈 A에 끼워 넣지 않음.
