...
복잡 파이프라인 시작 시 오케스트레이터가 첫 번째로 호출함. 산출된 컨텍스트는 모든 하위 에이전트가 공유함.
...
`FALLBACK_USED: true`이면 오케스트레이터가 사용자에게 스택 명시를 요청한 후 재호출함.
